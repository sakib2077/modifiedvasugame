...
//         console.log("Loaded game data from Flutter:", data);

//         BlockPuzzle.ScoreManager.instance.setMaxScores(data.maxScore);
//         BlockPuzzle.StarsManager.instance.setStarsAmount(data.numStars, false);
//         BlockPuzzle.AchievementsManager.instance.loadAchievementsState(data.achievements);
//         BlockPuzzle.PowerupManager.instance.setInitialState(BlockPuzzle.PowerupType.BOMB, data.bombBought);
//         BlockPuzzle.PowerupManager.instance.setInitialState(BlockPuzzle.PowerupType.LIGHTNING, data.lightningBought);
//         BlockPuzzle.Settings.TUTORIAL_COMPLETED = data.tutorialCompleted;
//         BlockPuzzle.SoundController.instance.setSFXVolume(data.sfxVolume);
//         BlockPuzzle.SoundController.instance.setMusicVolume(data.musicVolume);
    ...
    class InscriptionEffect extends Phaser.Group {
        constructor(inscriptionLevel, x, y) {
            ...
            this.game.add.tween(this.sprite)
                .to({
                    alpha: 1
                }, 420, Phaser.Easing.Sinusoidal.Out, false)
                .to({
                    alpha: 1
                }, 250, Phaser.Easing.Sinusoidal.Out, false)
                .to({
                    alpha: 0
                }, 650, Phaser.Easing.Sinusoidal.In, false)
                .start()
                .onComplete.add(() => this.destroy());
            this.game.add.tween(this.sprite)
                .to({
                    y: -120
                }, 350, Phaser.Easing.Sinusoidal.Out, false)
                .to({
                    y: -140
                }, 800, Phaser.Easing.Linear.None, false)
                .start();
            this.game.add.tween(this.sprite.scale)
                .to({
                    x: 1,
                    y: 1
                }, 420, this.amplifiedBackOut, false)
                .to({
                    x: 1.25,
                    y: 1.25
                }, 850, Phaser.Easing.Sinusoidal.In, false, 50)
                .start();
    ...
    class BlockStarredView extends Phaser.Group {
        ...
        startAppearingTween(initialParent, targetBlock) {
            ...
            this.game.add.tween(this.star)
                .to({
                    angle: 0
                }, BlockPuzzle.Settings.STARRED_BLOCK_APPEARING_TWEEN_DURATION * 0.62, Phaser.Easing.Sinusoidal.Out, false)
                .start();
            this.game.tweens.removeFrom(this.star.scale);
            ...
            this.game.add.tween(this.star.scale)
                ...
                .to({
                    x: 0.9,
                    y: 0.9
                }, BlockPuzzle.Settings.STARRED_BLOCK_APPEARING_TWEEN_DURATION * 0.1, Phaser.Easing.Sinusoidal.In, false)
                .start()
                .onComplete.add(() => {
    ...
    class Block extends Phaser.Group {
        ...
        deactivate(delay) {
            ...
            this.game.add.tween(this.blockGrayscale)
                .to({
                    alpha: 1
                }, BlockPuzzle.Settings.BLOCK_DEACTIVATION_TWEEN_DURATION, Phaser.Easing.Sinusoidal.Out, true, delay)
                .onComplete.add(() => {
                    this.block.visible = false;
                    if (this.hasStar()) {
        ...
        findExplosionStartingPosition(explodingCells, targetCells, pickRandom) {
            ...
            if (pickRandom) {
                let {
                    posX,
                    posY
                } = Phaser.ArrayUtils.getRandomItem(targetCellsToExplode);
                return {
                    x: posX,
                    y: posY
                };
    ...
    class FigureManager extends BlockPuzzle.AbstractManager {
        ...
        getPointerLocalPosition() {
            const {
                x,
                y
            } = this.figuresContainer.toLocal(this.game.input.activePointer.position, this.game.world);
        ...
        getAvailableBoardBounds() {
            if (BlockPuzzle.CustomScaleManager.isPortrait()) {
                const {
                    ...
                    minY,
                    maxY
                } = this.level.windowBounds.getBounds(0, 1, 0.12, 0.75);
                ...
                const {
                    ...
                    minY,
                    maxY
                } = this.level.windowBounds.getBounds(0, 0.75, 0.15, 1);
                return new Phaser.Rectangle((minX + maxX) / 2, (minY + maxY) / 2, maxX - minX, maxY - minY);
                ...
                const {
                    ...
                    minY,
                    maxY
                } = this.level.windowBounds.getBounds(0.2, 0.8, 0, 1);
                return new Phaser.Rectangle((minX + maxX) / 2, (minY + maxY) / 2, maxX - minX, maxY - minY);
            }
        }
        getAvailableFiguresPanelBounds() {
            if (BlockPuzzle.CustomScaleManager.isPortrait()) {
                let {
                    ...
                    minY,
                    maxY
                } = this.level.windowBounds.getBounds(0, 1, 0.75, 0.94);
                ...
                let {
                    ...
                    minY,
                    maxY
                } = this.level.windowBounds.getBounds(0.75, 1, 0.15, 0.87);
                ...
                let {
                    ...
                    minY,
                    maxY
                } = this.level.windowBounds.getBounds(0.8, 1, 0.16, 1);
                minX = this.level.boardManager.getBoardBounds(this.level.container).right;
                return new Phaser.Rectangle((minX + maxX) / 2, (minY + maxY) / 2, maxX - minX, maxY - minY);
            }
        }
        getAvailableUIContainerBounds() {
            if (BlockPuzzle.CustomScaleManager.isPortrait()) {
                let {
                    ...
                    minY,
                    maxY
                } = this.level.windowBounds.getBounds(0, 1, 0, 0.15);
                ...
                let {
                    ...
                    minY,
                    maxY
                } = this.level.windowBounds.getBounds(0, 0.2, 0.25, 0.72);
        ...
        getAvailablePowerupsContainerBounds() {
            if (BlockPuzzle.CustomScaleManager.isPortrait()) {
                let {
                    ...
                    minY,
                    maxY
                } = this.level.windowBounds.getBounds(0, 1, 0.9, 1);
                ...
                let {
                    ...
                    minY,
                    maxY
                } = this.level.windowBounds.getBounds(0.8, 1, 0.86, 0.95);
                ...
                let {
                    ...
                    minY,
                    maxY
                } = this.level.windowBounds.getBounds(0, 0.2, 0.86, 0.94);
        ...
        playAppearingTween(delay) {
            ...
            this.game.add.tween(this.scale)
                .to({
                    x: 1,
                    y: 1
                }, BlockPuzzle.Settings.FIGURE_APPEARING_DURATION, Phaser.Easing.Back.Out, true, delay)
...
(function(BlockPuzzle) {
    class ScoreManager {
        constructor() {
            ...
            this.maxScores = this.loadMaxScores(); // Load max score from localStorage
        ...
        updateMaxScores(currentScores) {
            if (currentScores > this.maxScores) {
                ...
                this.saveMaxScores(); // Save the updated max score to localStorage
                BlockPuzzle.LocalStorageController.instance.save();
                return true;
            }
            return false;
         ...
         */
        saveMaxScores() {
            localStorage.setItem('rr_highScore', this.maxScores);
        }
    
        loadMaxScores() {
            const savedScore = localStorage.getItem('rr_highScore');
    ...
    class TransitionScreen extends Phaser.Group {
        ...
        show() {
            ...
            this.game.add.tween(this.background)
                .to({
                    alpha: 1
                }, 140, Phaser.Easing.Quadratic.Out, true)
...
(function(BlockPuzzle) {
    class ButtonClaimReward extends Phaser.Group {
        constructor(panel, reward, initialScale, actionHandler, actionHandlerContext) {
            ...
            this.backplate["overTween"] = BlockPuzzle.App.instance.add.tween(this.container.scale).to({
                x: 1.05,
                y: 1.05
            }, 100);
            this.backplate["outTween"] = BlockPuzzle.App.instance.add.tween(this.container.scale).to({
                x: 1,
                y: 1
            }, 100);
            this.backplate["downTween"] = BlockPuzzle.App.instance.add.tween(this.container.scale).to({
                x: 0.95,
                y: 0.95
            }, 50).to({
                x: 1,
        ...
        animateAppearing(delay) {
            ...
            this.game.add.tween(this.textFieldLeft.scale)
                .to({
                    x: 0.75,
                    y: 0.75
                }, 450, Phaser.Easing.Back.Out, true, delay + 250)
            ...
            this.game.add.tween(this.textFieldRight.scale)
                .to({
                    x: 0.75,
                    y: 0.75
                }, 450, Phaser.Easing.Back.Out, true, delay + 330)
            ...
            this.game.add.tween(this.textFieldLeft)
                .to({
                    alpha: 1
                }, 500, Phaser.Easing.Linear.None, true, delay + 250)
                .onStart.add(() => this.textFieldLeft.visible = true);
            this.textFieldRight.visible = false;
            ...
            this.game.add.tween(this.textFieldRight)
                .to({
                    alpha: 1
                }, 500, Phaser.Easing.Linear.None, true, delay + 330)
                .onStart.add(() => this.textFieldRight.visible = true);
        }
        resetTweens() {
            ...
            if (duration < 400) {
                this.game.add.tween(this.icon.scale)
                    .to({
                        x: 1.075,
                        y: 1.075
                    }, 100, Phaser.Easing.Sinusoidal.Out, false)
                    .to({
                        x: 1.0,
                        y: 1.0
                    }, 240, Phaser.Easing.Sinusoidal.In, false)
                    .start();
            } else {
                this.game.add.tween(this.icon.scale)
                    .to({
                        x: 1.13,
                        y: 1.13
                    }, .25 * duration, Phaser.Easing.Sinusoidal.Out, false)
                    .to({
                        x: 1.15,
                        y: 1.15
                    }, 0.1 * duration, Phaser.Easing.Sinusoidal.In, false)
                    .to({
                        x: 1.0,
                        y: 1.0
                    }, 0.65 * duration, Phaser.Easing.Sinusoidal.In, false)
                    .start();
            }
        }
    }
        ...
        show(delay, starsAmount) {
            ...
            this.game.add.tween(this.textField.scale)
                .to({
                    x: 0.7,
                    y: 0.7
                }, 300, Phaser.Easing.Back.Out, true, delay + 200)
        ...
        collapse() {
            ...
            this.game.add.tween(this.buttonContainer.scale)
                .to({
                    x: 0,
                    y: 0
                }, 200, Phaser.Easing.Back.In, true, 100)
...
(function(BlockPuzzle) {
    class ScoreCounter extends BlockPuzzle.AbstractUICounter {
        ...
        buildChildren() {
            ...
            this.lastHighscoreValue = BlockPuzzle.ScoreManager.instance.getMaxScores();
            this.highscoreIcon = this.highscoreGroup.add(this.game.make.sprite(1, 20, BlockPuzzle.Settings.GAME_ATLAS, 'topScoreCrown0000'));
            ...
            this.highscoreText = this.highscoreGroup.add(new BlockPuzzle.BitmapTextField('' + this.lastHighscoreValue, BlockPuzzle.Settings.GAME_ATLAS, 'Gold', 1, 0, 0.2, 1));
    ...
    class ShopStarsCounter extends Phaser.Group {
        ...
        displayError() {
            ...
            this.game.add.tween(this.errorSmile)
                .to({
                    alpha: 0.9
                }, 120, Phaser.Easing.Sinusoidal.In, false)
                .to({
                    alpha: 0
                }, 700, Phaser.Easing.Quintic.In, false)
                .start();
            this.game.add.tween(this.errorSmile)
                .to({
                    y: 2
                }, 180, Phaser.Easing.Sinusoidal.In, false)
                .to({
                    y: -30
                }, 700, Phaser.Easing.Linear.None, false)
                .start();
            this.game.add.tween(this.errorSmile.scale)
                .to({
                    x: 1.5,
                    y: 1.5
                }, 240, Phaser.Easing.Elastic.Out, false)
                .to({
                    x: 1.65,
                    y: 1.65
                }, 550, Phaser.Easing.Quadratic.In, false)
                .start();
        }
    }
        ...
        playStarIncreasingTween() {
            ...
            this.game.add.tween(this.icon.scale)
                .to({
                    x: 1.45,
                    y: 1.45
                }, 80, Phaser.Easing.Sinusoidal.Out, false)
                .to({
                    x: 0.87,
                    y: 0.87
                }, 200, Phaser.Easing.Sinusoidal.In, false)
                .to({
                    x: 1,
                    y: 1
                }, 60, Phaser.Easing.Sinusoidal.Out, false)
                .start();
        }
        dispatchStarAdded(starsAmount) {
            this.updateStarsAmount(starsAmount);
        ...
        handleMultiplierChange(multiplier, progress) {
            if (this.storedMultiplierValue == multiplier) {
                ...
                this.game.add.tween(this.padWhite)
                    .to({
                        alpha: 0.5
                    }, 250, Phaser.Easing.Linear.None, false)
                    .to({
                        alpha: 0.0
                    }, 250, Phaser.Easing.Linear.None, false)
                    .start();
                ...
                this.game.add.tween(this.progressMask)
                    ...
                    }, fillingDuration, Phaser.Easing.Sinusoidal.Out, false)
                    .to({
                        y: -1 + (0.5) * this.padHeight
                    }, 1, Phaser.Easing.Sinusoidal.Out, false)
                    .to({
                        y: -1 + (0.5 - progress) * this.padHeight
                    }, totalDuration - fillingDuration + 1, Phaser.Easing.Sinusoidal.Out, false)
                    .start();
                ...
                this.game.add.tween(this.padWhite)
                    .to({
                        alpha: 0.6
                    }, fillingDuration, Phaser.Easing.Linear.None, false)
                    .to({
                        alpha: 0
                    }, totalDuration - fillingDuration, Phaser.Easing.Linear.None, false)
                    .start();
            }
        ...
        update() {
            ...
            if (this.level.gameStateManager.isGameActive() && this.dragging) {
                const {
                    x,
                    y
                } = this.toLocal(this.game.input.activePointer.position, this.game.world);
    ...
    class ResultsScoresPanel extends Phaser.Group {
        ...
        animateAppearing(delay) {
            ...
            this.game.add.tween(this.highscoreGroup)
                .to({
                    alpha: 1
                }, 300, Phaser.Easing.Sinusoidal.Out, true, delay + 400)
    ...
    class ReviveCountdown extends Phaser.Group {
        ...
        startCountdown() {
            ...
            this.game.add.tween(this.currentStep)
                .to({
                    value: 1
                }, BlockPuzzle.Settings.REVIVE_TIMER_DURATION, Phaser.Easing.Linear.None, true)
        ...
        startHeartbeatAnimation() {
            ...
            const heartbeatTween = this.game.add.tween(this.heart.scale)
                .to({
                    x: 1.1,
                    y: 1.1
                }, 100, Phaser.Easing.Sinusoidal.In, false, 0)
                .to({
                    x: 1,
                    y: 1
                }, 310, Phaser.Easing.Sinusoidal.Out, false)
                .to({
                    x: 1.1,
                    y: 1.1
                }, 100, Phaser.Easing.Sinusoidal.In, false, 25)
                .to({
                    x: 1,
                    y: 1
                }, 140, Phaser.Easing.Sinusoidal.Out, false)
                .to({
                    x: 1,
                    y: 1
                }, 650, Phaser.Easing.Sinusoidal.Out, false)
                .start()
                .repeatAll(-1);
        ...
        animateDisappearing(delay) {
            ...
            this.game.add.tween(this)
                .to({
                    alpha: 0
                }, 200, Phaser.Easing.Linear.None, true, delay + 50)
                .onComplete.add(() => {
            ...
            this.game.add.tween(this.circle)
                .to({
                    alpha: 0
                }, 200, Phaser.Easing.Sinusoidal.In, true, delay)
    ...
    class WindowHeading extends Phaser.Group {
        ...
        animateDisappearing(delay) {
            ...
            this.game.add.tween(this.container)
                .to({
                    y: -450
                }, 350, Phaser.Easing.Back.In, true, delay)
...
(function(BlockPuzzle) {
    class LocalStorageController {
        ...
        save() {
            ...
            /**
             * Whenever save tell dart to save the data
             */
            ScoreChannel.postMessage(JSON.stringify(this.data));
        }
        ...
        loadSave() {
            ...
            /**
             * Key to save data take the data from index.html
             */
            if(saveDataFromDart !== undefined) this.data = saveDataFromDart
    ...
    class CloseableWindow extends BlockPuzzle.DefaultWindow {
        ...
        animateDisappearing(delay, onClosedCallback = null) {
            ...
            this.game.add.tween(this.background)
                .to({
                    alpha: 0
                }, 300, Phaser.Easing.Circular.In, true, delay + 100)
                .onComplete.add(() => {
                    if (onClosedCallback)
                        onClosedCallback();
                    this.hide();
